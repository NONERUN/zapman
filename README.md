...
- [**`zapret.bat`**](./zapret.bat) — проверка среды и запуск GUI. Единственный `.bat` в корне.
- [**`strategies/`**](./strategies/) — стратегии обхода (`.ps1`). Выбираются в GUI. Пробуйте ALT, FAKE и другие, пока не найдёте рабочее решение.
- [**`utils/gui.ps1`**](./utils/gui.ps1) — окно: Start/Stop, Install Service, Remove Services, Game Filter, IPSet.
- [**`utils/service.ps1`**](./utils/service.ps1) — консольное меню (диагностика, тесты, hosts, обновления). Запуск: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File utils\service.ps1`
  - **`Install Service`** — стратегия в автозапуск (services.msc)
  - **`Remove Services`** — удаление стратегии и WinDivert из служб
  - **`Check Status`** — статус обхода и служб
  - **`Game Filter`** — обход для игр (UDP/TCP порты выше 1023). После смены нужен перезапуск; если служба установлена — Install Service снова.
  - **`IPSet Filter`** — `none` / `loaded` / `any`
  - **`Auto-Update Check`**, **`Replace active fakes`**, **`Update IPSet List`**, **`Update Hosts File`**, **`Check for Updates`**
  - **`Run Diagnostics`**, **`Run Tests`** (сайты из `utils/targets.txt` и DPI checkers)

Те же Start/Stop, служба и фильтры доступны в GUI.
...
Запустите **`zapret.bat`**, либо `utils\service.ps1` → **`Update Hosts File`**. После чего, если ваш hosts будет неактуальным, то Вам будет предложено обновить его самостоятельно:  
...
- Проверьте, чтобы не было ошибок в `utils\service.ps1` → `Run Diagnostics`
...
- `zapret.bat` → **Remove Services** (или `utils\service.ps1`)
- `utils\service.ps1` → `Run Diagnostics` (если есть ошибки - устраните их) -> в конце Y
...
- Как найдёте рабочую стратегию, поставьте её на автозапуск: GUI → **Install Service**
...
### При удалении через GUI / `utils\service.ps1` WinDivert остается в службах
...
- Запустите `utils\service.ps1` -> `Run Diagnostics` и выполните там очистку кэша Discord.
...
- через GUI или `utils\service.ps1` обновите ipset и включите `Game Filter`
...
- **`ipset-exclude-user.txt`** для исключения IP и подсетей
  - Файлы **`*-user.txt`** автоматически создадутся при первом запуске `zapret.bat`

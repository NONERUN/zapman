...
--filter-tcp=80,443 --hostlist="$lists\list-general.txt" --hostlist="$user\list-general-user.txt" --hostlist-exclude="$lists\list-exclude.txt" --hostlist-exclude="$user\list-exclude-user.txt" --ipset-exclude="$lists\ipset-exclude.txt" --ipset-exclude="$user\ipset-exclude-user.txt" --dpi-desync=multisplit --dpi-desync-split-pos=2,sniext+1 --dpi-desync-split-seqovl=679 --dpi-desync-split-seqovl-pattern="$bin\tls_clienthello_www_google_com.bin" --new
...
--filter-tcp=80,443,8443 --ipset="$user\ipset-all.txt" --hostlist-exclude="$lists\list-exclude.txt" --hostlist-exclude="$user\list-exclude-user.txt" --ipset-exclude="$lists\ipset-exclude.txt" --ipset-exclude="$user\ipset-exclude-user.txt" --dpi-desync=syndata --new
--filter-tcp=$($gf.Tcp) --ipset="$user\ipset-all.txt" --ipset-exclude="$lists\ipset-exclude.txt" --ipset-exclude="$user\ipset-exclude-user.txt" --dpi-desync=syndata --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n4 --new

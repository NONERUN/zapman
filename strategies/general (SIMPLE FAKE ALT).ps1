...
--filter-tcp=$($gf.Tcp) --ipset="$lists\ipset-all.txt" --ipset-exclude="$lists\ipset-exclude.txt" --ipset-exclude="$lists\ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n3 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=2 --dpi-desync-fake-tls="$bin\stun2.bin" --dpi-desync-fake-tls="$bin\tls_clienthello_www_google_com.bin" --dpi-desync-fake-http="$bin\tls_clienthello_max_ru.bin" --dpi-desync-fake-unknown="$bin\stun2.bin" --dpi-desync-fake-unknown="$bin\tls_clienthello_www_google_com.bin" --new

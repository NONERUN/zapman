...
--filter-tcp=$($gf.Tcp) --ipset="$lists\ipset-all.txt" --ipset-exclude="$lists\ipset-exclude.txt" --ipset-exclude="$lists\ipset-exclude-user.txt" --dpi-desync=fake --dpi-desync-repeats=6 --dpi-desync-any-protocol=1 --dpi-desync-cutoff=n5 --dpi-desync-fooling=ts --dpi-desync-fake-tls="$bin\stun2.bin" --dpi-desync-fake-tls="$bin\tls_clienthello_max_ru.bin" --dpi-desync-fake-http="$bin\tls_clienthello_max_ru.bin" --dpi-desync-fake-unknown="$bin\stun2.bin" --dpi-desync-fake-unknown="$bin\tls_clienthello_max_ru.bin" --new
